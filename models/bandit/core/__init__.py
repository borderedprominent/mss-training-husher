# Inference-only import: the training machinery below (LightningSystem,
# parse_*_config, etc.) requires pytorch_lightning / torchmetrics / asteroid.
# For Husher we only need `model` (the BSRNN wrapper) to be importable.
# If the training deps are missing, import only `model` and skip the rest.
import importlib.util as _importlib_util

_MISSING = [
    name for name in ("pytorch_lightning", "torchmetrics", "asteroid")
    if _importlib_util.find_spec(name) is None
]

if _MISSING:
    from models.bandit.core import model  # noqa: F401
else:
    import os.path
